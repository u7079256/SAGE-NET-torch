...
from io_helper import read_image, normalize
from stitching import stitch_together
from tensor_manipulation import resize_tensor, crop_tensor
...
def sample_signature(sequences, allowed_frames, image_size, allow_mirror):
    ...
    if force_sample_steering:
        ...
        p[[i for i in range(0, len(steering_dirs)) if steering_dirs[i] == 'STRAIGHT']] = prob_straight
        p[[i for i in range(0, len(steering_dirs)) if steering_dirs[i] == 'LEFT']] = prob_left
        p[[i for i in range(0, len(steering_dirs)) if steering_dirs[i] == 'RIGHT']] = prob_right
    p *= mask
    p /= np.sum(p)
    start = np.random.choice(list(range(0, total_frames_each_run)), p=p)
...
def visualize_batch(X, Y):
    ...
    for b in range(0, batchsize):
        for f in range(0, frames_per_batch):
            ...
            # stitch and visualize
            stitch_c = stitch_together([normalize(x_c), of_c, seg_c, normalize(y_c)], layout=(2, 2),
                                       resize_dim=(540, 960))
...
def test_load_batch():
    ...
    print()
    'Batch loaded in {} seconds.'.format(elapsed)
    print()
    'I shape:{}'.format(X[0].shape)
    print()
    'I_s shape:{}'.format(X[1].shape)
    print()
    'I_c shape:{}'.format(X[2].shape)
    print()
    'OF shape:{}'.format(X[3].shape)
    print()
    'OF_s shape:{}'.format(X[4].shape)
    print()
    'OF_c shape:{}'.format(X[5].shape)
    print()
    'SEG shape:{}'.format(X[6].shape)
    print()
    'SEG_s shape:{}'.format(X[7].shape)
    print()
    'SEG_c shape:{}'.format(X[8].shape)
    print()
    'Y shape:{}'.format(Y[0].shape)
    print()
    'Y_c shape:{}'.format(Y[1].shape)
